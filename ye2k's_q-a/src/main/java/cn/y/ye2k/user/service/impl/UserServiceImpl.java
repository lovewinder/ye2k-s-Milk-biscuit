...
public class UserServiceImpl implements IUserService {
	...
	public void userRegister(User user,String email) {
       ...
       if (findUser!=null){
		...
		//注册成功就发送邮箱激活链接
        try {
            MailUtil.sendMail("caoleicode@foxmail.com","<h2>尊敬的用户:"+user.getUserName()+",您已经成功注册,请点击本链接激活账户!如非本人操作,请勿忽视此邮件!</h2><a href='http://localhost:8080/user/active?code=user.getCode()"+user.getPassword()+"'>点击激活></a>");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
